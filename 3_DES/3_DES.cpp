...
public:
	...
	void Generate_Obj(byte *obj_bytes, std::string type)
	{
		if (type == "Key")
		{
			...
			rand.GenerateBlock(obj_bytes, CryptoPP::DES::DEFAULT_KEYLENGTH);
			for (int i = 0; i < CryptoPP::DES::DEFAULT_KEYLENGTH; i++)
			{
		...
		{
			CryptoPP::AutoSeededRandomPool rand;
			rand.GenerateBlock(obj_bytes, CryptoPP::DES::BLOCKSIZE);
			for (int i = 0; i < CryptoPP::DES::BLOCKSIZE; i++)
			{
...
public:
	...
	void Generate_Obj(byte *obj_bytes, std::string type)
	{
		...
		{
			CryptoPP::AutoSeededRandomPool rand;
			rand.GenerateBlock(obj_bytes, CryptoPP::DES_EDE3::DEFAULT_KEYLENGTH);
			for (int i = 0; i < CryptoPP::DES_EDE3::DEFAULT_KEYLENGTH; i++)
			{
				key.GetData().push_back(obj_bytes[i]);
			}
		...
		{
			CryptoPP::AutoSeededRandomPool rand;
			rand.GenerateBlock(obj_bytes, CryptoPP::DES_EDE3::BLOCKSIZE);
			for (int i = 0; i < CryptoPP::DES_EDE3::BLOCKSIZE; i++)
			{
				iv.GetData().push_back(obj_bytes[i]);
			}
...
int main()
{
	...
	{
		Clear_Screen();
		std::cout << "Select algorithm (1 - DES, 2 - 3DES)" << std::endl;
		int algh_mode;
		int cipher_mode = 0;
		int begin_time;
		int end_time;
		...
		if (algh_mode == 1)
		{
			...
			std::cout << std::endl << "DES encryption and decryption completed. Time = " << process_time << " seconds." << std::endl;
		}
		...
		else if (algh_mode == 2)
		{
			Clear_Screen();
			begin_time = clock();
			cipher_mode = obj3.Encryption();
			obj3.Decryption(cipher_mode);
			end_time = clock();
			std::string process_time = std::to_string(Get_Time(begin_time, end_time));
			std::cout << std::endl << "DES3 encryption and decryption completed. Time = " << process_time << " seconds." << std::endl;
		}
